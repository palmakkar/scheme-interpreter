...
def make_call_frame(procedure, args, env):
    ...
    # BEGIN Question 12
    if not isinstance(procedure, MuProcedure):
...
def do_if_form(expressions, env):
    ...
    expressions = expressions.second
    true_result = expressions.first
    if not scheme_false(condition):
        return scheme_eval(true_result, env)
    else:
        if expressions.second:
            false_result = expressions.second.first
        else:
            false_result = okay
        return scheme_eval(false_result, env)
    # END Question 13

def do_and_form(expressions, env, result = True):
    """Evaluate a short-circuited and form."""
    # BEGIN Question 14B
    if expressions:
        first = scheme_eval(expressions.first, env)
        if scheme_false(first):
            return False
        else:
            return do_and_form(expressions.second, env, first)
    else:
        return result
    # END Question 14B
...
def make_let_frame(bindings, env):
    ...
    def make_pairs():
        ...
        formals1 = bindings.first.first
        values1 = scheme_eval(bindings.first.second.first, env)
        bindings = bindings.second
        formals_next , values_next = make_pairs()
        return Pair(formals1, formals_next), Pair(values1, values_next)
    formals, vals = make_pairs()
    check_formals(formals)
...
def check_formals(formals):
    ...
    formalist = []
    while formals:
        if scheme_symbolp(formals.first) and formals.first not in formalist:
            formalist.append(formals.first)
        else:
            raise SchemeError
